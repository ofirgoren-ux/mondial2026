const matchDatabase = {
    'match1': {
        timeStatus: 'past', matchday: 1, stage: 'A', dateText: "בית א' | 9 ביוני",
    ...
    'match2': {
        ...
        probabilities: { home: 40, draw: 30, away: 30 }, matchRisk: 'Upset Alert',
    ...
    'match3': {
        timeStatus: 'past', matchday: 1, stage: 'B', dateText: "בית ב' | 10 ביוני",
    ...
    'match5': {
        timeStatus: 'past', matchday: 1, stage: 'C', dateText: "בית ג' | 11 ביוני",
    ...
    'match7': {
        timeStatus: 'past', matchday: 1, stage: 'D', dateText: "בית ד' | 12 ביוני",
    ...
    'match9': {
        timeStatus: 'past', matchday: 1, stage: 'E', dateText: "בית ה' | 13 ביוני",
    ...
    'match11': {
        ...
        probabilities: { home: 50, draw: 28, away: 22 }, matchRisk: 'Upset Alert',
    ...
    'match15': {
        ...
        probabilities: { home: 75, draw: 18, away: 7 }, matchRisk: 'Upset Alert',
    ...
    'match16': {
        ...
        insight: { prediction: "ביאלסה ואורוגוואי יביאו לחץ גבוה שערב הסעודית לא תוכל לעמוד בו.", actual: "אורוגוואי התעייפה משמעותית בסיום, מה שאפשר מהפך סעודי מדהים." },
    ...
    'match17': {
        ...
        insight: { prediction: "העומק של צרפת פשוט גדול מדי על ההגנה הסנגלית.", actual: "פגיעה במגמה! צרפת ניצחה בצדק עם שליטה מוחלטת ב-xG." },
    ...
    'match18': {
        ...
        probabilities: { home: 18, draw: 22, away: 60 }, matchRisk: 'Safe',
        ...
        insight: { prediction: "ההתאמה הטקטית הרסנית לעיראק. המודל צופה חגיגה נורווגית באוויר.", actual: "התפוצצות נורווגית וצמד של הולאנד סגרו עניין." },
    ...
    'match19': {
        ...
        insight: { prediction: "בוסטר תיקו מופעל! אלג'יריה היא קבוצת בונקר/מתפרצות.", actual: "המודל פישל מול גאונות. לאונל מסי סירב להיכנע לבוסטר והבקיע שלושער אדיר!" },
    ...
    'match20': {
        timeStatus: 'past', matchday: 1, stage: 'J', dateText: "בית י' | 18 ביוני",
        ...
        insight: { prediction: "הלחץ הגבוה של האוסטרים יחנק את בניית המשחק הירדנית.", actual: "אוסטריה שלטה, ירדן ניסתה להפתיע, אבל פנדל של ארנאוטוביץ' בסיום סגר עניין." },
    ...
    'match21': {
        ...
        teamHome: { name: 'אנגליה', flagCode: 'gb-eng', color: '#FFFFFF', cards: { yellow: ["סטונס (70')"], red: [] } },
        teamAway: { name: 'קרואטיה', flagCode: 'hr', color: '#FF0000', cards: { yellow: [], red: [] } },
        score: { prediction: '1 - 1', actual: '4 - 2', accuracyClass: 'wrong' },
        probabilities: { home: 45, draw: 35, away: 20 }, matchRisk: 'Draw Booster',
        radarStats: { home: [85, 80, 85, 85, 80, 98], away: [85, 80, 70, 90, 75, 80] },
        advancedStats: { home: { xG: 1.40, restDays: 5, altitudeImpact: 'רגיל' }, away: { xG: 1.10, restDays: 5, altitudeImpact: 'רגיל' } },
        insight: { prediction: "מוקש ענק לאנגלים. מרכז השדה הקרואטי המנוסה יגרור את אנגליה למאבק חפירות עיקש.", actual: "התפוצצות אנגלית! למרות התחזית למשחק צמוד, אנגליה רמסה את קרואטיה עם רבעייה אדירה." },
        goals: [{ team: 'אנגליה', player: "הארי קיין", minute: "12'" }, { team: 'אנגליה', player: "ג'וד בלינגהאם", minute: "30'" }, { team: 'קרואטיה', player: "אנדריי קראמריץ'", minute: "41'" }, { team: 'אנגליה', player: "פיל פודן", minute: "65'" }, { team: 'קרואטיה', player: "לוקה מודריץ'", minute: "78'" }, { team: 'אנגליה', player: "הארי קיין", minute: "88'" }],
        squads: { home: { predicted: ["פיקפורד", "בלינגהאם", "קיין"], actual: ["פיקפורד", "בלינגהאם", "קיין"] }, away: { predicted: ["ליבאקוביץ'", "מודריץ'", "קראמריץ'"], actual: ["ליבאקוביץ'", "מודריץ'", "קראמריץ'"] } }
    },
    'match22': {
        timeStatus: 'past', matchday: 1, stage: 'K', dateText: "בית כ' | 17 ביוני",
        teamHome: { name: 'פורטוגל', flagCode: 'pt', color: '#006600', cards: { yellow: ["פלהיניה (40')"], red: [] } },
        teamAway: { name: 'קונגו', flagCode: 'cg', color: '#FCD116', cards: { yellow: ["מאקוטה (80')"], red: [] } },
        ...
        advancedStats: { home: { xG: 2.80, restDays: 5, altitudeImpact: 'קל' }, away: { xG: 0.50, restDays: 5, altitudeImpact: 'בינוני' } },
        insight: { prediction: "פערי הרמות כאן משמעותיים מדי. פורטוגל תשייט לניצחון.", actual: "סנסציה אדירה! פורטוגל שלטה ללא עוררין ב-xG, אבל קונגו עקצה בדקה ה-89 עם שער הירואי." },
        goals: [{ team: 'פורטוגל', player: "ברונו פרננדש", minute: "15'" }, { team: 'קונגו', player: "איסק מבנזה", minute: "89'" }],
        squads: { home: { predicted: ["קושטה", "פרננדש", "רונאלדו"], actual: ["קושטה", "פרננדש", "רונאלדו"] }, away: { predicted: ["נדזילה", "גנבולה", "מבנזה"], actual: ["נדזילה", "גנבולה", "מבנזה"] } }
    },
    'match23': {
        timeStatus: 'past', matchday: 1, stage: 'L', dateText: "בית ל' | 17 ביוני",
        teamHome: { name: 'גאנה', flagCode: 'gh', color: '#CE1126', cards: { yellow: ["סאליסו (50')"], red: [] } },
        teamAway: { name: 'פנמה', flagCode: 'pa', color: '#DA291C', cards: { yellow: [], red: [] } },
        score: { prediction: '2 - 1', actual: '1 - 0', accuracyClass: 'trend' },
        probabilities: { home: 50, draw: 30, away: 20 }, matchRisk: 'Upset Alert',
        radarStats: { home: [75, 55, 60, 80, 85, 45], away: [65, 50, 65, 70, 75, 30] },
        advancedStats: { home: { xG: 1.60, restDays: 5, altitudeImpact: 'רגיל' }, away: { xG: 0.90, restDays: 5, altitudeImpact: 'רגיל' } },
        insight: { prediction: "גאנה פייבוריטית בזכות הפיזיות שלה, אבל פנמה מסוכנת מאוד במתפרצות.", actual: "פגיעה במגמה! גאנה עמדה בלחץ ושמרה על רשת נקייה בדרך לניצחון קטן וחשוב." },
        goals: [{ team: 'גאנה', player: "מוחמד קודוס", minute: "42'" }],
        squads: { home: { predicted: ["אטי-זיגי", "פארטיי", "קודוס"], actual: ["אטי-זיגי", "פארטיי", "קודוס"] }, away: { predicted: ["מוסקרה", "קראסקייה", "דיאז"], actual: ["מוסקרה", "קראסקייה", "דיאז"] } }
    },
    'match24': {
        timeStatus: 'past', matchday: 1, stage: 'K', dateText: "בית כ' | 17 ביוני",
        ...
        radarStats: { home: [60, 50, 60, 70, 75, 20], away: [85, 75, 80, 85, 85, 80] },
        advancedStats: { home: { xG: 0.60, restDays: 5, altitudeImpact: 'קשיי אקלים' }, away: { xG: 2.10, restDays: 5, altitudeImpact: 'אופטימלי' } },
        insight: { prediction: "קולומביה מגיעה עם סגל עדיף בהרבה טכנית ופיזית.", actual: "פגיעה במגמה! קולומביה שלטה וניצחה בהפרש של שני שערים כפי שציפינו, למרות שער כבוד אוזבקי." },
        goals: [{ team: 'קולומביה', player: "לואיס דיאז", minute: "25'" }, { team: 'אוזבקיסטן', player: "אלדור שומורודוב", minute: "40'" }, { team: 'קולומביה', player: "חאמס רודריגס", minute: "60'" }, { team: 'קולומביה', player: "ג'ון דוראן", minute: "82'" }],
        squads: { home: { predicted: ["יוסופוב", "שומורודוב", "מאשריפוב"], actual: ["יוסופוב", "שומורודוב", "מאשריפוב"] }, away: { predicted: ["ורגאס", "רודריגס", "דיאז"], actual: ["ורגאס", "רודריגס", "דיאז"] } }
    }
};