...
// =========================================================================
// מודול חלונית סגל מתקדם - עיצוב סייבר קוהרנטי ומתוקן
// =========================================================================
...
window.openTeamSquadModal = async function(selectedTeamName, selectedTeamFlag, oppTeamName) {
    ...
    if (!modal) {
        ...
        style.innerHTML = `
            ...
            /* המספר מימין לחלוטין עם רווח מספיק מהשם */
            .squad-player-num { background: var(--accent-cyan, #00e5ff); color: #000; font-weight: bold; width: 28px; height: 28px; display: flex; justify-content: center; align-items: center; border-radius: 4px; margin-left: 15px; font-size: 0.9rem; flex-shrink: 0; }
            
            /* אזור השם והתפקיד באמצע */
            ...
            .captain-badge { color: #fbbf24; font-weight: bold; font-size: 0.8rem; margin-right: 6px; display: inline-flex; align-items: center; justify-content: center; background: rgba(251, 191, 36, 0.1); width: 20px; height: 20px; border-radius: 50%; border: 1px solid rgba(251, 191, 36, 0.3); }
        ...
        if (teamLineup) {
            ...
            const getPlayerIcons = (playerName, isSubInCheck = false) => {
                ...
                eventsData.response.forEach(e => {
                    ...
                    // Subs In (checking e.assist safely to prevent the loop crash)
                    if (e.team.id === apiTeamId && e.assist && e.assist.name === playerName && e.type === 'subst' && isSubInCheck) {
                        let min = e.time.elapsed + (e.time.extra ? '+' + e.time.extra : '') + "'";
